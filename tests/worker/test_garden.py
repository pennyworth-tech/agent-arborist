...
def test_find_next_task_returns_first_ready(git_repo):
    tree = _make_tree()
    task = find_next_task(tree, git_repo, spec_id="main")
...
def test_garden_executes_task_successfully(git_repo, mock_runner_all_pass):
    tree = _make_tree()
    result = garden(tree, git_repo, mock_runner_all_pass, spec_id="main")
...
def test_garden_writes_report(git_repo, mock_runner_all_pass, tmp_path):
    ...
    garden(tree, git_repo, mock_runner_all_pass, report_dir=report_dir, spec_id="main")
...
def test_garden_retries_on_test_failure(git_repo, mock_runner_all_pass):
    ...
    result = garden(tree, git_repo, mock_runner_all_pass, test_command=test_cmd, max_retries=3, spec_id="main")
    assert result.success


def test_garden_fails_after_max_retries(git_repo, mock_runner_always_reject):
    tree = _make_tree()
    result = garden(tree, git_repo, mock_runner_always_reject, max_retries=2, spec_id="main")
...
def test_deep_tree_all_leaves_complete(git_repo, mock_runner_all_pass):
    ...
    # Complete T001
    r1 = garden(tree, git_repo, mock_runner_all_pass, spec_id="main")
    assert r1.success

    # Complete T003
    r2 = garden(tree, git_repo, mock_runner_all_pass, spec_id="main")
    assert r2.success

    # Complete T002
    r3 = garden(tree, git_repo, mock_runner_all_pass, spec_id="main")
    assert r3.success

    # All three tasks should be complete
    assert is_task_complete("T001", git_repo, spec_id="main")
    assert is_task_complete("T002", git_repo, spec_id="main")
    assert is_task_complete("T003", git_repo, spec_id="main")
...
def test_review_log_trailer_in_commit(git_repo, tmp_path):
    ...
    result = garden(tree, git_repo, runner, max_retries=3, log_dir=log_dir, spec_id="main")
    assert result.success
...
def test_test_log_trailer_in_commit(git_repo, tmp_path):
    ...
    result = garden(tree, git_repo, runner, test_command=test_cmd, max_retries=3, log_dir=log_dir, spec_id="main")
    assert result.success
...
def test_runner_timeout_passed_to_runner(git_repo, tmp_path):
    ...
    result = garden(tree, git_repo, runner, max_retries=3, runner_timeout=120, spec_id="main")
    assert result.success
...
def test_runner_timeout_default_when_none(git_repo, tmp_path):
    ...
    result = garden(tree, git_repo, runner, max_retries=3, spec_id="main")
    assert result.success
