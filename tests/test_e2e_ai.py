...
Validates git structure: commits with correct format, changed files, all commits
on the base branch. Does NOT check specific file contents or outputs.
...
def test_ai_garden_creates_commits_with_trailers(git_repo, runner_type, model, cli_binary):
    ...
    # Commits follow task(main@T001@...) convention on HEAD
...
def test_ai_gardener_completes_all_tasks(git_repo, runner_type, model, cli_binary):
    ...
    first_t001 = next(i for i, s in enumerate(subjects) if "task(main@T001@" in s)
    first_t002 = next(i for i, s in enumerate(subjects) if "task(main@T002@" in s)
    assert first_t001 < first_t002


# ---------------------------------------------------------------------------
# 4. Per-node test commands with real AI: verify trailers
# ---------------------------------------------------------------------------
