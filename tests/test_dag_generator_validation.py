...
class TestFixEnvFormat:
    ...
    def test_already_correct_format(self):
        ...
        dag = {"env": ["ARBORIST_MANIFEST=${DAG_DIR}/test.json"]}
        fixes = _fix_env_format(dag)

        assert dag["env"] == ["ARBORIST_MANIFEST=${DAG_DIR}/test.json"]
        assert len(fixes) == 0
    ...
    def test_removes_duplicates(self):
        ...
        dag = {"env": [
            "ARBORIST_MANIFEST=${DAG_DIR}/test1.json",
            "ARBORIST_MANIFEST=${DAG_DIR}/test2.json",
        ]}
        fixes = _fix_env_format(dag)

        assert len(dag["env"]) == 1
        assert dag["env"][0] == "ARBORIST_MANIFEST=${DAG_DIR}/test1.json"
...
class TestValidateAndFixDag:
    ...
    def test_real_world_dag_structure(self):
        ...
        dag = {
            ...
            "env": [
                "ARBORIST_MANIFEST=${DAG_DIR}/hello_world.json",
                "ARBORIST_MANIFEST=${DAG_DIR}/hello-world.json",
            ],
            "steps": [
