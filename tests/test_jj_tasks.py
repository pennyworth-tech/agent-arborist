...
class TestRunJJ:
    ...
    def test_run_jj_success(self):
        ...
        with patch("agent_arborist.jj_tasks.is_jj_installed", return_value=True):
            with patch("subprocess.run") as mock_run:
                ...
                with patch("agent_arborist.jj_tasks.get_git_root", return_value=Path("/tmp")):
                    result = run_jj("log")
    ...
    def test_run_jj_not_installed(self):
        ...
        with patch("agent_arborist.jj_tasks.is_jj_installed", return_value=False):
...
class TestCreateChange:
    ...
    def test_create_change(self):
        ...
        with patch("agent_arborist.jj_tasks.run_jj"):
            with patch("agent_arborist.jj_tasks.get_change_id", return_value="newchange"):
    ...
    def test_create_task_change(self):
        ...
        with patch("agent_arborist.jj_tasks.create_change", return_value="taskchange") as mock_create:
...
class TestSquashAndRebase:
    ...
    def test_squash_into_parent_success(self):
        ...
        with patch("agent_arborist.jj_tasks.run_jj") as mock_run:
            mock_run.return_value = MagicMock(returncode=0, stdout="", stderr="")
            with patch("agent_arborist.jj_tasks.has_conflicts", return_value=False):
    ...
    def test_squash_with_conflicts(self):
        ...
        with patch("agent_arborist.jj_tasks.run_jj") as mock_run:
            mock_run.return_value = MagicMock(returncode=0, stdout="", stderr="")
            with patch("agent_arborist.jj_tasks.has_conflicts", return_value=True):
                result = squash_into_parent("child", "parent")
                assert result.success is True
...
class TestWorkspaceManagement:
    ...
    def test_get_workspace_path(self, tmp_path, monkeypatch):
        ...
        from agent_arborist import jj_tasks
        monkeypatch.setattr(jj_tasks, "get_arborist_home", lambda: tmp_path)
    ...
    def test_workspace_exists_true(self):
        ...
        with patch("agent_arborist.jj_tasks.list_workspaces", return_value=["default", "ws-T001"]):
            assert workspace_exists("ws-T001") is True

    def test_workspace_exists_false(self):
        """Returns False for non-existent workspace."""
        with patch("agent_arborist.jj_tasks.list_workspaces", return_value=["default"]):
...
class TestTaskLifecycle:
    ...
    def test_complete_task(self):
        ...
        with patch("agent_arborist.jj_tasks.get_description", return_value="spec:123:T001"):
            with patch("agent_arborist.jj_tasks.describe_change"):
                with patch("agent_arborist.jj_tasks.squash_into_parent") as mock_squash:
    ...
    def test_sync_parent_no_conflicts(self):
        ...
        with patch("agent_arborist.jj_tasks.has_conflicts", return_value=False):
            with patch("agent_arborist.jj_tasks.find_pending_children", return_value=["child1"]):
                with patch("agent_arborist.jj_tasks.rebase_change") as mock_rebase:
    ...
    def test_sync_parent_with_conflicts(self):
        ...
        with patch("agent_arborist.jj_tasks.has_conflicts", return_value=True):
            with patch("agent_arborist.jj_tasks.get_description", return_value="spec:123:T002"):
                with patch("agent_arborist.jj_tasks.describe_change"):
                    with patch("agent_arborist.jj_tasks.find_pending_children", return_value=[]):
                        result = sync_parent("parent", "spec123")
...
class TestQueryFunctions:
    ...
    def test_find_task_change(self):
        ...
        with patch("agent_arborist.jj_tasks.run_jj") as mock_run:
            ...
            with patch("agent_arborist.jj_tasks.get_description", return_value="spec:002:T001"):
                with patch("agent_arborist.jj_tasks.has_conflicts", return_value=False):
...
class TestInitColocated:
    ...
    def test_init_colocated_already_exists(self, tmp_path):
        ...
        with patch("agent_arborist.jj_tasks.get_git_root", return_value=tmp_path):
