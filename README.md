...
Git-native task tree orchestration. Grow your task tree and let AI execute it — our arborist tends to each branch (task) in turn.
...
This sends your spec to an AI planner (Claude Opus by default) which produces a `task-tree.json` with the full hierarchy and execution order. Each task includes test commands that are automatically run, and AI reviews are performed automatically after implementation.
